...
int
CacheContinuation::setupVCdataLink(int , void *)
{
  // not implement now
...
int
CacheContinuation::setupVCdataDeref(int , void *)
{
  ink_assert(!"not implement");
  return 0;
}
...
//  ClusterBufferReader *ret = new_ClusterBufferReader();
  ...
  Ptr<IOBufferData> head;
  head = new_IOBufferData(iobuffer_size_to_index(flen + msg->h_len));
...
cache_op_result_ClusterFunction(ClusterSession cs, void *context, void *d)
{
  ...
  ClusterCont *cc = (ClusterCont *) d;
  ink_assert(cvc && cc && cc->data_len > 0 && cvc->mutex->thread_holding == this_ethread());
    ...
    case CACHE_EVENT_OPEN_READ:
      {
        int h_len = msg->h_len;
        if (cc->data && h_len > 0) {
          ink_assert(cc->data_len >= h_len && cvc->frag_type == CACHE_FRAG_TYPE_HTTP);
          // big file or the hdr exceed one Bufferblock
          Ptr<IOBufferData> buf;
          buf = new_IOBufferData(iobuffer_size_to_index(h_len));
        ...
        cvc->d_len = msg->d_len;
        ink_assert(msg->d_len == cc->data_len);
        cvc->blocks = cc->data;
