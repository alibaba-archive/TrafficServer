...
{
  last_check_time = TV_INTER(ink_get_based_hrtime_internal());
}

void 
PriorityEventQueue::enqueue(Event * e, ink_hrtime now)
{
  unsigned long expires = TV_INTER(e->timeout_at) & T_MASK;
...
{ 
  ...
  while (time_after_eq(TV_INTER(now), last_check_time)) {
    now_idx = last_check_time & T_MASK & TVR_MASK;

    if (!now_idx && (!cascade(now, 2, INDEX(0))))
      cascade(now, 3, INDEX(1));

    while ((e = tv1[now_idx].dequeue())) {
